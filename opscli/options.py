...
def complete_options(options, words):
    results = []
    for opt in options:
...
def help_options(options, words):
    '''
        ...
        for case in cases:
            ...
            if result != expected:
                if not title:
                    ...
                    title = True
                print "    Failed %s: %s got %s instead of %s" % (case[0],
                        case[2], result, expected)
